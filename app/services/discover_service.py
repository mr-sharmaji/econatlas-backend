...
def _generate_metric_insights(row: dict, sector_stats: dict | None = None) -> dict:
    ...
    # Educational glossary: what each metric means (prepended to contextual insight)
    _gloss: dict[str, str] = {
        "pe_ratio": "Price-to-Earnings ratio tells you how many years of current earnings you're paying for.",
        "price_to_book": "Price-to-Book compares the stock price to the company's net asset value per share.",
        "eps": "Earnings Per Share is net profit divided by total shares — a key profitability metric.",
        "forward_pe": "Forward P/E uses expected future earnings (analyst estimates) rather than past earnings.",
        "peg_ratio": "PEG ratio adjusts the P/E for growth — it shows whether you're overpaying relative to growth.",
        "dividend_yield": "Dividend yield is the annual dividend as a percentage of the stock price.",
        "beta": "Beta measures how much the stock moves relative to the market (Nifty).",
        "market_cap": "Market cap is the total market value of all the company's shares.",
        "roe": "Return on Equity measures profit generated for every rupee of shareholder equity.",
        "roce": "Return on Capital Employed measures how efficiently the company uses all its capital (equity + debt).",
        "operating_margins": "Operating margin is profit from core operations as a percentage of revenue.",
        "profit_margins": "Net profit margin is the percentage of revenue retained after all expenses.",
        "revenue_growth": "Year-over-year percentage change in total revenue.",
        "earnings_growth": "Year-over-year percentage change in net profit.",
        "compounded_sales_growth_3y": "3-year revenue CAGR smooths out yearly fluctuations to show the underlying growth trajectory.",
        "compounded_profit_growth_3y": "3-year profit CAGR shows whether profitability is consistently improving.",
        "debt_to_equity": "Debt-to-Equity compares total borrowings to shareholder equity — lower means less leverage.",
        "interest_coverage": "Interest coverage shows how many times operating profit can cover interest payments.",
        "total_debt": "Total borrowings (short-term + long-term) the company owes.",
        "total_cash": "Cash and equivalents on the balance sheet — a safety cushion for downturns.",
        "payout_ratio": "The percentage of earnings paid out as dividends.",
        "free_cash_flow": "Cash left over after paying for operations and investments.",
        "cash_from_operations": "Cash generated by the company's core business activities.",
        "cash_from_investing": "Money spent on or received from long-term assets like factories and equipment.",
        "cash_from_financing": "Money raised from or returned to investors and lenders.",
        "rsi_14": "14-day RSI measures recent price momentum on a 0–100 scale.",
        "promoter_holding": "Percentage of shares held by the company's founders/promoters.",
        "fii_holding": "Percentage held by Foreign Institutional Investors (global funds, pension funds).",
        "dii_holding": "Percentage held by Domestic Institutional Investors (Indian mutual funds, LIC, etc.).",
        "score_financial_health": "Financial Health score measures ROE, ROCE, debt levels, and cash flow quality.",
        "score_valuation": "Valuation score evaluates whether the stock is fairly priced using P/E, PEG, and P/B.",
        "score_growth": "Growth score assesses revenue growth, earnings growth, and margin trends.",
        "score_momentum": "Momentum score tracks RSI, moving average trends, and breakout signals.",
        "score_smart_money": "Smart Money score analyzes FII, DII, and promoter ownership patterns.",
        "score_risk_shield": "Risk Shield score measures beta, pledged shares, debt, and earnings stability.",
    }

    def _add(key: str, context: str, sentiment: str) -> None:
        gloss = _gloss.get(key, "")
        explanation = f"{gloss} {context}".strip() if gloss else context
